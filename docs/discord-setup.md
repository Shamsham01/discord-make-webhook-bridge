...
This page is for server administrators who connect HookBot to Make and manage workflows. You do not need to write code.
...
The bot should appear in your member list as **HookBot**.

### Permissions HookBot needs
...
If a permission is missing, HookBot may be online but stay silent. Fix this under **Server Settings → Integrations** or **Roles**.

## Channel setup

HookBot works in any text channel it can see. Many servers use a dedicated channel such as `#ai-chat` or `#ask-bot`.

### Give HookBot access

In each channel where people should use HookBot:

1. Open channel settings → **Permissions**
2. Allow the HookBot role: **View Channel**, **Send Messages**, **Read Message History**
...
If you leave **channel** empty, the workflow works anywhere HookBot can read messages.

{% hint style="info" %}
If AI routing is on and the router is limited to a different channel than your default workflow, @HookBot in the default’s channel still works — HookBot falls back to the default when the router is not allowed there.
{% endhint %}
...
The **first** workflow you add becomes the **default** automatically. That is what @HookBot and replies use when AI routing is off.
...
If Make received the test, Discord shows a success message. A successful test means Make got the request — finish your Make scenario if HookBot still has nothing useful to post back.

## How people use HookBot

### @HookBot in a message

```
@HookBot Can you summarise this in three bullet points?
```

### Reply to @HookBot

Click **Reply** on a message HookBot sent. No need to @mention again.

```
Make that shorter and add a friendly greeting.
```

While working, HookBot may react with 👀 (if enabled). When done, it replies in the same channel.

## The default workflow

The **default** is the everyday automation for @HookBot and replies when AI routing is **off**.
...
| --- | --- |
| `@HookBot question` | Default workflow, or **router** first if AI routing is on |
| Reply to @HookBot | Same as above |
...
| `/webhook default` | Choose the workflow for @HookBot and replies |
...
* Run `/webhook list` if @HookBot does nothing — confirm a default is set
* Run `/webhook test` in the same channel where people @HookBot
* If slash commands are missing, re-invite HookBot with `bot` and `applications.commands` scopes
...
3. See [Hosting and troubleshooting](hosting-and-troubleshooting.md) if HookBot is offline or misbehaving
